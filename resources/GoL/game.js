...
function updateStatus() {
	PS.statusText( "Tickrate: " + tickrate + " | +/- to change | Age: " + age );
}
...
function updateAdjacent() {
	for (let i = 0; i < tiles.length; i++) {
		...
		for (let j = 0; j < element.length; j++) {
			const tile = element[j];
			numtiles[i][j] = countAdjacent(i, j);
		}
	}
}
...
function countAdjacent(x, y) {
	...
	if (tiles[leftx][upy]) count++;
	if (tiles[x][upy]) count++;
	if (tiles[rightx][upy]) count++;
	if (tiles[leftx][y]) count++;
	if (tiles[rightx][y]) count++;
	if (tiles[leftx][downy]) count++;
	if (tiles[x][downy]) count++;
	if (tiles[rightx][downy]) count++;
...
function initializeGrid() {
	tiles = Array.from({ length: PS.gridSize().width }, () => new Array(PS.gridSize().height).fill(false));
	numtiles = Array.from({ length: PS.gridSize().width }, () => new Array(PS.gridSize().height).fill(0));
}
